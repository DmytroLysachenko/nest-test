...
API_READ_THROTTLE_TTL_MS="${API_READ_THROTTLE_TTL_MS:-60000}"
API_READ_THROTTLE_LIMIT="${API_READ_THROTTLE_LIMIT:-120}"
API_WRITE_THROTTLE_TTL_MS="${API_WRITE_THROTTLE_TTL_MS:-60000}"
API_WRITE_THROTTLE_LIMIT="${API_WRITE_THROTTLE_LIMIT:-60}"
API_AUTH_THROTTLE_TTL_MS="${API_AUTH_THROTTLE_TTL_MS:-60000}"
API_AUTH_THROTTLE_LIMIT="${API_AUTH_THROTTLE_LIMIT:-10}"
API_SENSITIVE_THROTTLE_TTL_MS="${API_SENSITIVE_THROTTLE_TTL_MS:-60000}"
API_SENSITIVE_THROTTLE_LIMIT="${API_SENSITIVE_THROTTLE_LIMIT:-8}"
...
OPS_ALERTS_JOB_NAME="${OPS_ALERTS_JOB_NAME:-job-seek-dispatch-ops-alerts}"
OPS_ALERTS_CRON="${OPS_ALERTS_CRON:-*/30 * * * *}"
OPS_ALERTS_TIMEZONE="${OPS_ALERTS_TIMEZONE:-Europe/Warsaw}"
OPS_ALERTS_ATTEMPT_DEADLINE="${OPS_ALERTS_ATTEMPT_DEADLINE:-180s}"
...
TASKS_CALLER_SERVICE_ACCOUNT="${WORKER_TASKS_SERVICE_ACCOUNT_EMAIL:-$API_RUNTIME_SA}"
if [[ -n "${WORKER_SHARED_TOKEN:-}" ]]; then
  WORKER_TASK_AUTH_ENV_VARS="TASKS_AUTH_TOKEN=${WORKER_SHARED_TOKEN}|TASKS_SERVICE_ACCOUNT_EMAIL="
  API_WORKER_TASK_AUTH_ENV_VARS="WORKER_AUTH_TOKEN=${WORKER_SHARED_TOKEN}|WORKER_TASKS_SERVICE_ACCOUNT_EMAIL="
else
  WORKER_TASK_AUTH_ENV_VARS="TASKS_AUTH_TOKEN=|TASKS_SERVICE_ACCOUNT_EMAIL=${TASKS_CALLER_SERVICE_ACCOUNT}"
  API_WORKER_TASK_AUTH_ENV_VARS="WORKER_AUTH_TOKEN=|WORKER_TASKS_SERVICE_ACCOUNT_EMAIL=${TASKS_CALLER_SERVICE_ACCOUNT}"
fi

WORKER_BASE_ENV_VARS="^|^NODE_ENV=production|WORKER_ALLOWED_ORIGINS=${WORKER_ALLOWED_ORIGINS}|QUEUE_PROVIDER=cloud-tasks|TASKS_PROJECT_ID=${GCP_PROJECT_ID}|TASKS_LOCATION=${GCP_REGION}|TASKS_QUEUE=${WORKER_TASKS_QUEUE}|PLAYWRIGHT_HEADLESS=true|WORKER_MAX_CONCURRENT_TASKS=1|WORKER_MAX_QUEUE_SIZE=20|WORKER_TASK_TIMEOUT_MS=180000|${WORKER_TASK_AUTH_ENV_VARS}|WORKER_CALLBACK_TOKEN=${WORKER_CALLBACK_TOKEN}"
WORKER_LEGACY_SECRET_ENV_VARS="TASKS_AUTH_TOKEN,WORKER_CALLBACK_TOKEN,DATABASE_URL"

API_BASE_ENV_VARS=""
if [[ "$DEPLOY_API" == "true" ]]; then
  API_BASE_ENV_VARS="^|^NODE_ENV=production|HOST=0.0.0.0|DATABASE_URL=${DATABASE_URL}|ACCESS_TOKEN_SECRET=${ACCESS_TOKEN_SECRET}|ACCESS_TOKEN_EXPIRATION=${ACCESS_TOKEN_EXPIRATION}|REFRESH_TOKEN_SECRET=${REFRESH_TOKEN_SECRET}|REFRESH_TOKEN_EXPIRATION=${REFRESH_TOKEN_EXPIRATION}|MAIL_HOST=${MAIL_HOST}|MAIL_PORT=${MAIL_PORT}|MAIL_SECURE=${MAIL_SECURE}|MAIL_USERNAME=${MAIL_USERNAME}|MAIL_PASSWORD=${MAIL_PASSWORD}|GCS_BUCKET=${GCS_BUCKET}|GCP_PROJECT_ID=${GCP_PROJECT_ID}|GCP_LOCATION=${GCP_REGION}|GEMINI_MODEL=${GEMINI_MODEL}|GOOGLE_OAUTH_CLIENT_ID=${GOOGLE_OAUTH_CLIENT_ID}|GOOGLE_OAUTH_CLIENT_SECRET=${GOOGLE_OAUTH_CLIENT_SECRET}|API_PREFIX=api|API_READ_THROTTLE_TTL_MS=${API_READ_THROTTLE_TTL_MS}|API_READ_THROTTLE_LIMIT=${API_READ_THROTTLE_LIMIT}|API_WRITE_THROTTLE_TTL_MS=${API_WRITE_THROTTLE_TTL_MS}|API_WRITE_THROTTLE_LIMIT=${API_WRITE_THROTTLE_LIMIT}|API_AUTH_THROTTLE_TTL_MS=${API_AUTH_THROTTLE_TTL_MS}|API_AUTH_THROTTLE_LIMIT=${API_AUTH_THROTTLE_LIMIT}|API_SENSITIVE_THROTTLE_TTL_MS=${API_SENSITIVE_THROTTLE_TTL_MS}|API_SENSITIVE_THROTTLE_LIMIT=${API_SENSITIVE_THROTTLE_LIMIT}|WORKER_TASK_PROVIDER=cloud-tasks|WORKER_TASKS_PROJECT_ID=${GCP_PROJECT_ID}|WORKER_TASKS_LOCATION=${GCP_REGION}|WORKER_TASKS_QUEUE=${WORKER_TASKS_QUEUE}|${API_WORKER_TASK_AUTH_ENV_VARS}|WORKER_CALLBACK_TOKEN=${WORKER_CALLBACK_TOKEN}|SCHEDULER_AUTH_TOKEN=${SCHEDULER_AUTH_TOKEN}|OPS_INTERNAL_TOKEN=${OPS_INTERNAL_TOKEN}|OPS_ALERTS_WINDOW_HOURS=${OPS_ALERTS_WINDOW_HOURS:-24}|OPS_ALERTS_COOLDOWN_MINUTES=${OPS_ALERTS_COOLDOWN_MINUTES:-60}"

  if [[ -n "${OPS_ALERTS_WEBHOOK_URL:-}" ]]; then
    API_BASE_ENV_VARS="${API_BASE_ENV_VARS}|OPS_ALERTS_WEBHOOK_URL=${OPS_ALERTS_WEBHOOK_URL}"
  fi

  if [[ -n "${OPS_ALERTS_WEBHOOK_BEARER_TOKEN:-}" ]]; then
    API_BASE_ENV_VARS="${API_BASE_ENV_VARS}|OPS_ALERTS_WEBHOOK_BEARER_TOKEN=${OPS_ALERTS_WEBHOOK_BEARER_TOKEN}"
  fi
fi
API_LEGACY_SECRET_ENV_VARS="DATABASE_URL,ACCESS_TOKEN_SECRET,REFRESH_TOKEN_SECRET,MAIL_USERNAME,MAIL_PASSWORD,GOOGLE_OAUTH_CLIENT_SECRET,WORKER_AUTH_TOKEN,WORKER_CALLBACK_TOKEN,SCHEDULER_AUTH_TOKEN,OPS_INTERNAL_TOKEN,OPS_ALERTS_WEBHOOK_BEARER_TOKEN"
...
if [[ "$DEPLOY_API" == "true" ]]; then
  ...
  API_CALLBACK_URL="${API_URL%/}/api/job-sources/complete"
fi
...
if [[ "$DEPLOY_WORKER" == "true" ]]; then
  ...
  gcloud run deploy "$GCP_WORKER_SERVICE" \
    ...
    --memory="${WORKER_MEMORY}" \
    --remove-secrets="${WORKER_LEGACY_SECRET_ENV_VARS}" \
    --set-env-vars="${WORKER_BASE_ENV_VARS}|DATABASE_URL=${DATABASE_URL}|TASKS_URL=${WORKER_TASK_URL}|TASKS_OIDC_AUDIENCE=${WORKER_TASK_URL}|WORKER_CALLBACK_URL=${API_URL}/api/job-sources/complete" \
    >/dev/null
fi
...
if [[ "$DEPLOY_API" == "true" ]]; then
  ...
  if [[ -n "$FINAL_API_ALLOWED_ORIGINS" ]]; then
    ...
    gcloud run deploy "$GCP_API_SERVICE" \
      ...
      --memory=512Mi \
      --remove-secrets="${API_LEGACY_SECRET_ENV_VARS}" \
      --set-env-vars="${API_BASE_ENV_VARS}|ALLOWED_ORIGINS=${FINAL_API_ALLOWED_ORIGINS}|WORKER_TASK_URL=${WORKER_URL}/tasks|WORKER_TASKS_OIDC_AUDIENCE=${WORKER_URL}/tasks|WORKER_CALLBACK_URL=${API_CALLBACK_URL}" \
      >/dev/null
  fi
fi
...
if [[ "$DEPLOY_API" == "true" ]]; then
  ...
  if [[ -n "${OPS_ALERTS_WEBHOOK_URL:-}" ]]; then
    echo "Ensure Cloud Scheduler ops alert dispatch job..."
    upsert_scheduler_job \
      "$OPS_ALERTS_JOB_NAME" \
      "$OPS_ALERTS_CRON" \
      "$OPS_ALERTS_TIMEZONE" \
      "${API_URL}/api/ops/dispatch-alerts" \
      "$OPS_INTERNAL_TOKEN" \
      "$OPS_ALERTS_ATTEMPT_DEADLINE"
  else
    echo "Skip Cloud Scheduler ops alert dispatch job because OPS_ALERTS_WEBHOOK_URL is not configured."
  fi
fi
