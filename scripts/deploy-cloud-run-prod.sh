...
gcloud run deploy "$GCP_WORKER_SERVICE" \
  ...
  --set-env-vars="NODE_ENV=production,QUEUE_PROVIDER=cloud-tasks,TASKS_PROJECT_ID=${GCP_PROJECT_ID},TASKS_LOCATION=${GCP_REGION},TASKS_QUEUE=${WORKER_TASKS_QUEUE},TASKS_URL=https://example.com/tasks,PLAYWRIGHT_HEADLESS=true,WORKER_MAX_CONCURRENT_TASKS=1,WORKER_MAX_QUEUE_SIZE=20,WORKER_TASK_TIMEOUT_MS=180000" \
  >/dev/null

WORKER_URL="$(gcloud run services describe "$GCP_WORKER_SERVICE" --project="$GCP_PROJECT_ID" --region="$GCP_REGION" --format='value(status.url)')"
...
API_BASE_URL="$API_URL" WORKER_BASE_URL="$WORKER_URL" WEB_BASE_URL="$WEB_URL" ./scripts/verify-deployment.sh
