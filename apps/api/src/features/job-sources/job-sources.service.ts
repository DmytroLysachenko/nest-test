...
export class JobSourcesService {
  ...
  async getPreflight(userId: string, dto: EnqueueScrapeDto) {
    ...
    const catalogEligibleCount = profileContext.profile
      ? await this.countFreshCatalogMatchesForProfile({
          userId,
  ...
  private async persistAcceptedJobsForRun(
    run: {
      id: string;
      source: 'PRACUJ_PL';
      listingUrl: string;
    ...
    jobs: CallbackJobPayload[],
  ) {
    if (!jobs.length) {
      return { offerIds: [] as string[], offerIdentityKeys: [] as string[] };
    }

    const now = new Date();
    const jobsToPersist = await this.reuseExistingUrlsBySourceId(run.source, jobs);
    ...
    return {
      offerIds: persistedOffers.map((offer) => offer.id),
      offerIdentityKeys: persistedOffers.map((offer) => offer.offerIdentityKey),
    };
  }

  private async linkPersistedOffersForRun(
    run: {
      id: string;
      userId: string | null;
    ...
    if (!run.userId || !run.careerProfileId || !offerIds.length) {
      return { insertedCount: 0, totalCandidateCount: offerIds.length, matchedCount: 0 };
    }
    ...
    return this.linkCatalogOffersToUser({
      userId: run.userId,
      careerProfileId: run.careerProfileId,
      sourceRunId: run.id,
      source: run.source,
  ...
  private async countLinkedOffersForRun(runId: string) {
  ...
  > {
    ...
    if (profileContext.profile) {
      const freshProjection = await this.prepareLinkableCatalogOffers({
        userId: input.userId,
        careerProfileId: input.careerProfileId,
        source: input.source,
        profile: profileContext.profile,
  ...
  private async countFreshCatalogMatchesForProfile(input: {
    ...
    const prepared = await this.prepareLinkableCatalogOffers(input);
    return prepared.matchingOffers.length;
  }

  private async prepareLinkableCatalogOffers(input: {
    userId: string;
    careerProfileId: string;
    source: 'PRACUJ_PL';
    profile: CandidateProfile;
    ...
    return { candidates, matchingOffers };
  }
  ...
  private async linkCatalogOffersToUser(input: {
    ...
    const now = new Date();
    const { candidates, matchingOffers } = await this.prepareLinkableCatalogOffers(input);
    if (!candidates.length) {
